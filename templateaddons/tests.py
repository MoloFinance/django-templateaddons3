...
class AssignTemplateTagTestCase(TemplateTagTestCase):
    ...
    def test_output(self):
        ...
        fixtures = [
            ...
            (u'{% assign name="sample" %}1234{% endassign %}{% assign name="sample" %}5678{% endassign %}{{ sample }}',
             u'5678'),  # context override
...
class CounterTemplateTagTestCase(TemplateTagTestCase):
    ...
    def test_output(self):
        ...
        fixtures = [
            ...
            (u'{% counter name="c2" %}{% counter %}{% counter name="c2" %}{% counter name="c2" %}', u'0012'),  # NOQA name parameter NOQA
            (u'{% counter name="c1" %}{% counter name="c2" %}{% counter name="c1" %}{% counter name="c1" %}{% counter name="c2" %}',  # NOQA
             u'00121'),  # name parameter
            ...
            (u'{% counter ascending=0 step=-1 %}{% counter %}{% counter %}', u'012'),  # NOQA ascending parameter and negative step
            ...
            (u'{% counter assign="c1" %}{{ c1 }}{% counter %}{% counter assign="c1" %}{{ c1 }}{% counter %}{% counter assign="c2" %}{% counter %}{{ c1 }}{{ c2 }}', u'0012234524'),  # NOQA assign parameter
            (u'{% counter start=4 step=4 ascending=0 %}{% counter start=8 step=23 ascending=1 %}{% counter %}', u'40-4'),  # NOQA only first declaration affects step and ascending parameters
        ]
        # add template tag library to template code
...
class HeadingContextTemplateTagTestCase(TemplateTagTestCase):
    ...
    def test_output(self):
        ...
        fixtures = [
            ...
            (u'{% headingcontext source_level=2 target_level=4 %}<h2>Test</h2>{% endheadingcontext %}',
             u'<h4>Test</h4>'),
            (u'{% headingcontext source_level=5 target_level=4 %}<h5>Test</h5>{% endheadingcontext %}',
             u'<h4>Test</h4>'),
        ]
        # add template tag library to template code
...
# </html>""")),
        ...
        # add template tag library to template code
        fixtures = [(u'{% load javascript %}' + template_code, valid_output) for (template_code, valid_output) in fixtures]  # NOQA
        # test real output
        self.validate_template_code_result(fixtures)
    ...
    def test_output(self):
        ...
        fixtures = [
            ...
            (u'{% replace search="\s+" replacement="-" %}to\t \n\n   \tto{% endreplace %}',
             u'to-to'),  # regular expressions
            (u'{% replace search="(to)" replacement="\\1a" %}toto{% endreplace %}',
             u'toatoa'),  # NOQAbackreferences in regular expressions
            (u'{% replace search="([a-z]+)" replacement="*\\1*" %}123abc456def{% endreplace %}',
             u'123*abc*456*def*'),  # backreferences in regular expressions
            (u'{% replace search="(to)" replacement="au" %}(to)to{% endreplace %}', u'(au)au'),  # regexp not escaped
            (u'{% replace search="(to)" replacement="au" use_regexp=0 %}(to)to{% endreplace %}',
             u'auto'),  # escaped regexp
